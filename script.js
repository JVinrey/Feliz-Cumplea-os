...
const MASTER_TL = timeline({
  onStart: () => {
    SOUNDS.ON.play();
    // Mostrar el mensaje al inicio de la animación
    document.querySelector('.message-above').style.display = 'block';
  },
  onComplete: () => {
    delayedCall(2, () => {
      document.querySelector('.message-above').style.display = 'none';
      RESET();
    });
